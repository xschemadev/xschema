...
if (result.success) {
  console.log(result.data.id, result.data.email);
  //                    ^?
}
```
...
type ResolveKey<K extends string, DefaultNS extends string | undefined> = K extends `${string}:${string}` ? K : DefaultNS extends string ? `${DefaultNS}:${K}` : K;
export type XSchemaType<N extends SchemaTypeKeys> = Register extends { schemaTypes: infer T } ? N extends keyof T ? T[N] : never : never;
export type XSchemaConfig<T extends Record<string, unknown>> = { schemas?: T; defaultNamespace?: string };
export function createXSchemaClient<const T extends Record<string, unknown>, const DefaultNS extends string | undefined = undefined>(config: XSchemaConfig<T> & { defaultNamespace?: DefaultNS }): <K extends ValidKeys<T, DefaultNS>>(key: K) => T[ResolveKey<K, DefaultNS> & keyof T];
// @filename: xschema.gen.ts
// Type-only adapter generates SchemaTypes but no schemas
export const schemas = {} as const;
export type SchemaTypes = { "user:Profile": { id: string; name?: string; email: string } };
declare module '@xschemadev/client' { interface Register { schemas: typeof schemas; schemaTypes: SchemaTypes } }
// @filename: index.ts
// ---cut---
import type { XSchemaType } from "@xschemadev/client";
...
const schema = xschema("user:Proflie");
```
