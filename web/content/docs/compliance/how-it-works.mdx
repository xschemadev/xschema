...
import { Ban, Terminal, Plug, ShieldCheck } from "lucide-react";

xschema tests adapters against the official [JSON Schema Test Suite](https://github.com/json-schema-org/JSON-Schema-Test-Suite) to verify correctness. This page explains how the compliance testing pipeline works internally.
...
```
┌──────────────┐    ┌─────────────┐    ┌──────────┐    ┌──────────────┐
│ Fetch Suite  │───▶│ Load Tests  │───▶│  Bundle  │───▶│   Generate   │
└──────────────┘    └─────────────┘    └──────────┘    └──────────────┘
                                                              │
┌──────────────┐    ┌─────────────┐    ┌──────────┐          │
│    Report    │◀───│   Execute   │◀───│  Harness │◀─────────┘
└──────────────┘    └─────────────┘    └──────────┘
```
...
<Cards>
  <Card
    ...
    description="Detailed explanations of features that cannot be statically compiled"
    href="/docs/compliance/unsupported-features"
    icon={<Ban />}
  />
  <Card
    title="CLI Compliance Command"
    description="Full command reference for xschema compliance"
    href="/docs/cli/compliance"
    icon={<Terminal />}
  />
  <Card
    title="Building Adapters"
    description="Create a new adapter with compliance testing"
    href="/docs/adapters/building-adapters"
    icon={<Plug />}
  />
  <Card
    title="Compliance Overview"
    description="What compliance means for xschema"
