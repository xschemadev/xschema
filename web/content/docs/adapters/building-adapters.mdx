...
import { Terminal, Ban, ShieldCheck } from "lucide-react";
...
Adapters communicate with the Go CLI via stdin/stdout JSON:

```
CLI --[JSON array]--> stdin --> adapter --> stdout --[JSON array]--> CLI
```

### Input Format

The CLI sends a JSON array of `ConvertInput` objects:

```typescript
interface ConvertInput {
  namespace: string;  // e.g. "user"
  id: string;         // e.g. "Profile"
  varName: string;    // e.g. "userProfile" (safe for code)
  schema: object;     // Pre-bundled JSON Schema (draft2020-12)
}
```

### Output Format

Your adapter returns a JSON array of `ConvertResult` objects:

```typescript
interface ConvertResult {
  namespace: string;  // Echo back from input
  id: string;         // Echo back from input
  varName: string;    // Echo back from input
  imports: string[];  // Import statements needed
  schema?: string;    // Schema code (e.g. "z.string()")
  type?: string;      // Type expression (e.g. "z.infer<typeof varName>")
  validate?: string;  // Optional validation function
  validationImports?: string[]; // Imports for validation function
}
```

<Callout type="info">
At least one of `schema` or `type` must be provided. Use `schema` for runtime validators, `type` for type-only generation.
</Callout>
...
│   └── renderer.ts # IR -> Zod code (~900 lines)
├── package.json
...
<Cards>
  <Card
    title="CLI Compliance Command"
    description="Full reference for the compliance command"
    href="/docs/cli/compliance"
    icon={<Terminal />}
  />
  <Card
    title="Unsupported Features"
    description="Features that can't be statically compiled"
    href="/docs/compliance/unsupported-features"
    icon={<Ban />}
  />
  <Card
    title="Compliance Overview"
    description="Understanding the compliance system"
