/**
 * Generates the type probe fixture by running the zod adapter's convert function
 * against representative JSON schemas for each targeted construct.
 *
 * Run: bun run tasks/type-fidelity-baseline/zod/generate-probe.ts
 */
import { convert } from "../../../typescript/packages/adapters/zod/src/index.js";
...
const probeCases: ProbeCase[] = [
	// --- oneOf ---
	{
		...
		description: "oneOf with string | number - should ideally be string | number, currently any",
		schema: {
	...
	{
		name: "notString",
		description: "not string - should be unknown, currently any",
		schema: {
			not: { type: "string" },
		},
	},
	{
		name: "notBoolean",
		description: "not boolean - should be unknown, currently any",
		schema: {
			not: { type: "boolean" },
		},
	},
	...
	// --- conditional (if/then/else) ---
	{
		...
		description: "if/then/else - should be unknown, currently any",
		schema: {
	...
	},
	{
		...
		description: "if/then only - should be unknown, currently any",
		schema: {
	...
	// --- typeGuarded ---
	{
		...
		description: "type-guarded object properties without explicit type - currently any",
		schema: {
	...
	// --- tuple ---
	{
		...
		description: "tuple [string, number] - should be [string, number, ...any[]], currently any[]",
		schema: {
			type: "array",
	...
	},
	{
		...
		description: "closed tuple [string, number] - should be [string, number], currently any[]",
		schema: {
			type: "array",
			prefixItems: [{ type: "string" }, { type: "number" }],
			items: false,
	...
	},
	{
		...
		description: "tuple [string, number, ...boolean[]] - currently any[]",
		schema: {
			type: "array",
			prefixItems: [{ type: "string" }, { type: "number" }],
			items: { type: "boolean" },
	...
	{
		name: "constObject",
		description: "const object - should be narrow type, gets passthrough object",
		schema: {
	...
	{
		name: "constArray",
		description: "const array - should be narrow type, gets any[]",
		schema: {
			const: [1, 2, 3],
		},
	},
