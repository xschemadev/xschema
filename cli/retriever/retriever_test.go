...
func TestRetrieveFromURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := RetrieveFromURL(ctx, tt.url)
			if tt.wantErr {
				if err == nil {
					t.Error("expected error, got nil")
				}
...
func TestRetrieveAggregation(t *testing.T) {
	...
	batches, err := RetrieveWithContext(ctx, decls)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestRetrieveErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := RetrieveWithContext(ctx, tt.decls)
			if err == nil {
				t.Error("expected error, got nil")
			}
		})
...
func TestRetrieveContextCancellation(t *testing.T) {
	...
	_, err := RetrieveWithContext(ctx, decls)
	if err == nil {
